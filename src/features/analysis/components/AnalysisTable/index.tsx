...
export const AnalysisTable = <T extends Analysis>({
  ...
  return (
    <div className={className}>
      <div className="border-line-light bg-light-gray flex flex-col flex-wrap items-center justify-center gap-x-5 gap-y-2 rounded-t-sm border-b px-4 py-2 md:flex-row md:justify-start">
      ...
      <div className="max-w-full overflow-x-auto">
        {data.length > 0 && (
          <table className="w-full table-fixed bg-light text-center">
            <thead className="border-line-light border-b">
              ...
              {table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      key={cell.id}
                      className="border-line-light h-[2.375rem] w-28 border-b p-[0.36rem] text-xs font-medium uppercase"
                    >
                      {flexRender(
