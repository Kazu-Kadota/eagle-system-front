...
import { VehicleAnalysisType, type BackRegionPersonAnalysis } from '@/models';
...
export const requestAnalysisPerson = async (
  ...
  await requestAuth.post(env.NEXT_PUBLIC_API_REQUEST_URL, `/analysis/person`, {
    body,
  });
};
...
export const requestBasicVehicleAnalysis = async (
  ...
  await requestAuth.post(env.NEXT_PUBLIC_API_REQUEST_URL, path, { body });
};
